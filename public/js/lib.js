...
app.memberInit = function () {
  app.checkLocalToken();
};

app.checkLocalToken = function () {
  ...
  if (temp !== null) {
    ...
    xhr.onreadystatechange = function () {
      if (this.readyState === 4 && this.status === 200) {
        ...
        console.log(data.id);
        app.state.auth.userID = data.id;
      }
    };

    xhr.open("GET", `https://weimazon.online/api/1.0/user/profile`);
    xhr.setRequestHeader("Authorization", `Bearer ${temp}`);
    xhr.send();
  }

  app.state.auth = { accessToken: localStorage.getItem("accessToken") };
  // app.state.auth.userID = localStorage.getItem("userID");
};
...
app.fb.loginStatusChange = function (response) {
  if (response.status === "connected") {
    ...
    if (typeof app.fb.nativeProfileCallback === "function") {
      app.fb.nativeProfileCallback();
    }
  }
};
